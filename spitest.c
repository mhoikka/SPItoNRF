...
void receiveByteNRF(){
    ...
    unsigned char enabledPipes = 0x07; //each 1 in the binary representation of this number corresponds to an enabled pipe in range pipes 0-5
...
void enableDataPipes(unsigned char pipes){
    ...
    for (unsigned char pipe = 0; pipe < 6; pipe++){
        ...
        if (pipes & (1 << pipe)){ //if the pipe should enabled
            printf("%d", pipe);
            ...
            if(pipe == 0x00){
                ...
                if (pipe == 0x01){
                    ...
                    readwriteNRF_SPI(RX_ADDR_Px, rxAddress[2], 1, WRITE_REG_NRF, 0); //set read address for pipe
                }
            }
